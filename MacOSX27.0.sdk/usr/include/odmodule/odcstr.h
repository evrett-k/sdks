...
/*!
    @function   od_copy_cstr_from_cfstring
    @abstract   get a malloc'd copy of a CFString as a UTF8 C string
    @discussion Unlike od_cstr_from_cfstring, this always allocates a copy
                so the result is safe to use after the source CFString is
                released (e.g., after a lock protecting the CFString is dropped).
    @param      inCFStr the CFString to extract
    @param      ioCStr receives the malloc'd buffer; caller must free if non-NULL
    @result     a pointer to the malloc'd C string (same as *ioCStr), or NULL on failure
*/
OS_EXPORT OS_NOTHROW OS_WARN_RESULT OS_MALLOC
const char *
od_copy_cstr_from_cfstring(CFStringRef inCFStr, char **ioCStr);
