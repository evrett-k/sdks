...
/**
 * @brief A process called `bootstrap_check_in()` to register a named service
 ...
 *
 * @field instigator       (Optional) The process that called
 *                         `bootstrap_check_in()`. Best-effort; may be null if
 *                         the instigator exited before the event was
 *                         constructed.
 * @field instigator_token Audit token of the instigator, captured by launchd
 *                         at RPC time. Always present.
 * @field service_name     The name registered by the instigator.
 *
 * @note This event type does not support caching.
 */
typedef struct {
...
/**
 * @brief A process called `bootstrap_look_up()` to resolve a named service
 *        port registered with launchd. launchd returns a send right to
 *        that port; subsequent `mach_msg()` calls to it are delivered to
 *        the owner of the corresponding receive right.
 *
 *        Submitted by launchd on behalf of the instigator.
 *
 *        Because launchd is the submitter, the enclosing message's
 ...
 *                         constructed.
 * @field instigator_token Audit token of the instigator, captured by
 *                         launchd at RPC time. Always present.
 * @field service_name     The name the instigator asked launchd to resolve.
 * @field target_type      Discriminator for the `target` union. Selects
 *                         between a running owner (PROCESS) and a
 *                         lazy-launched owner (JOB).
 * @field target           Identity of the entity that would receive
...
typedef struct {
	...
	union {
		struct {
			es_process_t *_Nullable target;
			audit_token_t target_token;
			es_string_token_t job_label;
		} process;
		struct {
			es_string_token_t job_label;
			es_lightweight_code_requirement_t *_Nullable lwcr;
		} job;
	} target;
...
typedef union {
	...
	es_event_bootstrap_check_in_t *_Nonnull bootstrap_check_in;
	es_event_bootstrap_look_up_t *_Nonnull bootstrap_look_up;
 ...
 * @param msg The message to be retained
 *
 * @note It is necessary to retain a message when the es_message_t provided
 *       in the event handler block of `es_new_client` will be processed
 *       asynchronously.
 *
 * @note Each call to es_retain_message must be balanced by exactly one call
 *       to es_release_message. Failing to release a retained message leaks
 *       memory. Releasing a message more than once per retain, or releasing
 *       a message that was never retained, is undefined behavior.
 */
OS_EXPORT
...
/**
 * Releases a message previously retained with `es_retain_message`.
 *
 * @param msg The message to be released
 *
...
/**
 * @brief Get the dictionary of entitlements associated with a message containing an es_event_exec_t
 *
 * @param exec The es_event_exec_t being inspected
 * @return The XPC dictionary containing all entitlements, or NULL if there are no entitlements.
 *
...
API_UNAVAILABLE(ios)
API_UNAVAILABLE(tvos, watchos) _Nullable xpc_object_t es_exec_entitlements(const es_event_exec_t *_Nonnull exec);
