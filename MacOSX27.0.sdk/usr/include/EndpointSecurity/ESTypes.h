...
/**
 * @brief Discriminator for the `target` union of
...
typedef enum {
	...
	// The following events are available beginning in macOS 27.0
	ES_EVENT_TYPE_AUTH_XPC_CONNECT,

	// The following events are available beginning in macOS 27.0 / iOS 27.0
...
/**
 * @brief Identity facts cached from a service's Lightweight Code
 *        Requirement (LWCR) — the code-signing constraint launchd was
 *        told to enforce when the service binary spawns.
 *
 * Used as a nullable pointer on the JOB arm of
 * `es_event_bootstrap_look_up_t.target`: NULL means launchd had no
 * LWCR cached for this service, distinct from "LWCR was present but
 * carried no team_id / signing_id" (which would be a non-NULL pointer
 * with the corresponding string token's `data` field set to NULL).
 *
 * The PROCESS arm omits this struct entirely — the receiving process's
 * actual code-signing identity is available via
 * `target.process.target->signing_id` / `->team_id` (kernel-sourced
 * from cs_ops on the audit token).
 *
 * Both string token fields preserve the three-state distinction common
 * to optional ES strings: `data == NULL` (the LWCR did not carry this
 * fact), `data != NULL && length == 0` (carried an empty value), and
 * `data != NULL && length > 0` (carried a value).
 *
 * @field team_id     Team Identifier from the LWCR. The token's `data`
 *                    is NULL when the LWCR did not carry this fact.
 * @field signing_id  Signing Identifier from the LWCR. The token's
 ...
 *
 * Determines how the system responds when a client fails to respond to an auth event
 * within the deadline.
 */
typedef enum {
