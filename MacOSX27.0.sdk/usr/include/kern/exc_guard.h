...
/*
 * Return a human-readable string for a guard type.
 */
#define EXC_GUARD_NAME_FOR_TYPE(guard_type) \
	(((guard_type) == GUARD_TYPE_NONE)        ? "NONE" : \
	 ((guard_type) == GUARD_TYPE_MACH_PORT)   ? "MACH_PORT" : \
	 ((guard_type) == GUARD_TYPE_FD)          ? "FD" : \
	 ((guard_type) == GUARD_TYPE_USER)        ? "USER" : \
	 ((guard_type) == GUARD_TYPE_VN)          ? "VNODE" : \
	 ((guard_type) == GUARD_TYPE_VIRT_MEMORY) ? "VIRT_MEMORY" : \
	 ((guard_type) == GUARD_TYPE_REJECTED_SC) ? "REJECTED_SC" : \
	 ((guard_type) == GUARD_TYPE_FROZEN_SWAPIN) ? "FROZEN_SWAPIN" : \
	 "UNKNOWN")
