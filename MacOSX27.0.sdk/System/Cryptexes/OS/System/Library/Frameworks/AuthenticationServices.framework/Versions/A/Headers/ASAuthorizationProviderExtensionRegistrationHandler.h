...
typedef NS_OPTIONS(NSUInteger, ASAuthorizationProviderExtensionRequestOptions) {
    ...
    // The device uses fallback for OpenID.
    ASAuthorizationProviderExtensionRequestOptionsOpenIDFallback API_UNAVAILABLE(macos) API_UNAVAILABLE(ios, watchos, tvos) = 1 << 7,
};
