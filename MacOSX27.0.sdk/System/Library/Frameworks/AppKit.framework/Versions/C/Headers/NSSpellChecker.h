...
APPKIT_EXTERN NSTextCheckingOptionKey NSTextCheckingAutomaticCapitalizationEnabledKey API_AVAILABLE(macos(26.0));  // Number evaluated as boolean, to be used to override the user preference given by automaticCapitalizationEnabled
APPKIT_EXTERN NSTextCheckingOptionKey NSTextCheckingWaitForAllGrammarCheckingResultsKey API_AVAILABLE(macos(27.0));  // Number evaluated as boolean, to be used in the options passed to requestCheckingOfString to specify that the request wait to return until all grammar analysis has been performed; if this is not specified, then the request will include those grammar results that are available quickly
...
- (void)ignoreGrammarRange:(NSRange)grammarRange inSentence:(NSString *)sentence inSpellDocumentWithTag:(NSInteger)tag API_AVAILABLE(macos(27.0));
