/* iig(DriverKit-509.0.1) generated from OSSerialization.iig */

/* OSSerialization.iig:1-40 */
/*
...
typedef kern_return_t (^OSSerializationPortCopyInHandler)(uint64_t portRef, OSObject ** obj);
typedef kern_return_t (^OSSerializationPortCopyOutHandler)(OSObject * obj, uint64_t * portRef);

/* source class OSSerialization OSSerialization.iig:41-126 */
...
public:
	 ...
	 *              OSData, OSString, OSNumber, OSBoolean, OSArray, OSDictionary.
     * @param       copyOutHandler An optional handler that allows an object to be replaced with a port name before being serialized
	...
	createFromObject(const OSObjectPtr object, OSSerializationPortCopyOutHandler copyOutHandler);

    /*!
     * @brief       Similar to the above variant, except assuming copyOutHandler to be NULL.
     */
	static OSSerializationPtr
     ...
     * @param       copyInHandler An optional handler that allows a port number to be replaced with an OSObject instance during unserialization
     * @return      NULL on failure, otherwise the allocated OSSerialization with reference count 1 to be released by the caller.
     */
	static OSSerializationPtr
	createFromBytes(const void * bytes, size_t length, OSSerializationFreeBufferHandler freeBuffer, OSSerializationPortCopyInHandler copyInHandler);

    /*!
     * @brief       Similar to the above variant, except assuming copyInHandler to be NULL.
     */
	static OSSerializationPtr
...
/* generated class OSSerialization OSSerialization.iig:41-126 */
...
\
    ...
    createFromObject(\
        ...
        OSSerializationPortCopyOutHandler copyOutHandler);\
\
    static OSSerializationPtr\
    createFromObject(\
        const OSObjectPtr object);\
\
    ...
    createFromBytes(\
        ...
        OSSerializationPortCopyInHandler copyInHandler);\
\
    static OSSerializationPtr\
    createFromBytes(\
        const void * bytes,\
