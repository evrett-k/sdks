...
enum {
	...
	CSSLOT_SIGNATURESLOT2 = 0x10003,                /* Second CMS Signature */
	...
	CS_CDHASH_LEN = 20,                                             /* legacy compatibility - full hashes available via new APIs */
	CS_HASH_MIN_SIZE = 20, /* minimum valid hash size (SHA-1) */
	CS_HASH_MAX_SIZE = 64, /* max size of the hash we'll support (SHA-512) */
 ...
 *
 * Carries the full variable-length cdhash returned by CS_OPS_CDHASH_WITH_INFO.
 * The hash[] buffer is sized to CS_HASH_MAX_SIZE; `length` reports the number
 * of valid bytes (32 for SHA256, 48 for SHA384, etc.).
 */

typedef struct _csops_cdhash {
	uint8_t hash[CS_HASH_MAX_SIZE];
	uint8_t type;                  /* Hash type (cdHashType constants) */
	uint8_t length;                /* Valid bytes in hash[] */
} csops_cdhash_t;
