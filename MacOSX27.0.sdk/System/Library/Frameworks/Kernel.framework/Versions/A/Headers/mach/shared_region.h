...
#define SHARED_REGION_BASE_ARM64                0x180000000ULL /* 6 GiB */
#define SHARED_REGION_SIZE_ARM64                0xE40000000ULL /* 57 GiB */
...
#if __LP64__
/* written out to start_address by shared_region_check_np() */
typedef union {
	/*
	 * When errno == ENOMEM, the base/size of the shared region placeholder is
	 * packed into a uintptr_t and written out (arm64 only).
	 */
	struct {
		uintptr_t size_gb : 17; /* Maximum size of the shared region in GiB (including ASLR) */
		uintptr_t base_addr : 47; /* Base address of the shared region (pre-ASLR) */
	};
	/*
	 * When successful, shared_region_check_np() writes out the address of the
	 * first mapping in the shared region
	 */
