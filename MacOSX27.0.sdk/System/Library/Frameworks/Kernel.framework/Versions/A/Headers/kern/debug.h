...
__options_closed_decl(kf_override_flag_t, uint32_t, {
	...
	KF_COMPRESSOR_AGE_TRACKING_OVRD           = 0x40000,
});
...
__options_decl(eph_panic_flags_t, uint64_t, {
	...
	EMBEDDED_PANIC_HEADER_FLAG_DEVICE_INFO_READ_FAILED        = 0x80000,                        /* ERROR: Reading device information from EDT failed */
});

#define MAX_PANIC_INITIATOR_SIZE 256

#define EMBEDDED_PANIC_HEADER_CURRENT_VERSION 9
#define EMBEDDED_PANIC_MAGIC 0x46554E4B /* FUNK */
#define EMBEDDED_PANIC_HEADER_OSVERSION_LEN 32
#define EMBEDDED_PANIC_HEADER_KERNELVERSION_LEN 512
#define EMBEDDED_PANIC_HEADER_PRODUCT_LEN 32

/*
 * Processor Panic Timing Tracking System
 *
 * This system tracks the timing of each processor during panic handling,
 * providing visibility into the duration of each step in the panic
 * coordination sequence, IPI handling, and coredump generation.
 */

typedef enum {
	PANIC_TIMING_DEBUGGER_LOCK_ACQUIRED = 0,                 /* Acquired debugger lock (panic leader) */
	PANIC_TIMING_IPI_SENT,                                   /* Successfully sent debug IPI */
	PANIC_TIMING_CORES_HALTED,                               /* Successfully halted (normal or forced) */
	PANIC_TIMING_PANIC_INFO_START,                           /* Started panic log creation */
	PANIC_TIMING_STACKSHOT_START,                            /* Started stackshot collection */
	PANIC_TIMING_STACKSHOT_FINISHED,                         /* Completed stackshot collection */
	PANIC_TIMING_PANIC_INFO_FINISHED,                        /* Created panic info (panic log and stackshot) */
	PANIC_TIMING_COREDUMP_START,                             /* Started coredump generation */
	PANIC_TIMING_COREDUMP_KERNEL_DUMP_FINISHED,              /* Completed kernel coredump */
	PANIC_TIMING_COREDUMP_SECURE_KERNEL_DUMP_FINISHED,       /* Completed secure kernel coredump */
	PANIC_TIMING_COREDUMP_COPROCESSORS_DUMP_FINISHED,        /* Completed coprocessor coredumps */
	PANIC_TIMING_COREDUMP_USERSPACE_DUMP_FINISHED,           /* Completed userspace coredumps */
	PANIC_TIMING_COREDUMP_FINISHED,                          /* Completed coredump successfully */
	PANIC_TIMING_MAX
} panic_timing_t;

/* Coprocessor coredump status */
#define COPROC_NAME_MAX_LEN 6

typedef struct coproc_status_entry {
	char name[COPROC_NAME_MAX_LEN + 1];         /* Coprocessor identifier name (null-terminated) */
	kern_return_t status;                       /* Coredump status (KERN_SUCCESS, KERN_FAILURE, etc.) */
} coproc_status_entry_t;

/* Panic timing entry for linked list */
typedef struct panic_timing_entry {
	panic_timing_t timing_type;                 /* Type of timing event */
	uint64_t timestamp;                         /* Timestamp when event occurred */
	uint32_t next_offset;                       /* Offset to next entry in linked list (0 if last) */
} panic_timing_entry_t;

/* Panic information tracker - tracks timing and coprocessor coredump status */
#define PANIC_INFO_TRACKER_MAGIC_V1 0x50414E49 /* 'PANI' - Version 1 */

typedef struct {
	uint32_t pit_magic_version;                 /* Magic number with embedded version */
	/* Timing timeline - linked list in panic region */
	uint32_t pit_timing_offset;                 /* Offset from panic region base to first timing entry */
	/* Coprocessor coredump status tracking - preallocated array in panic region */
	uint32_t pit_coproc_status_offset;          /* Offset from panic region base to first coprocessor entry */
	uint32_t pit_coproc_count;                  /* Total number of coprocessors registered */
} panic_info_tracker_t;


/* Function declarations for panic timing tracking */
extern void panic_timing_update(panic_timing_t timing);

/* Function declaration for coprocessor status tracking */
extern void panic_coproc_status_update(const char *coproc_name, kern_return_t status);
...
struct embedded_panic_header {
	...
	uint32_t eph_soc_id;
	uint32_t eph_soc_rev;
	char eph_product[EMBEDDED_PANIC_HEADER_PRODUCT_LEN];
	char eph_kernel_version[EMBEDDED_PANIC_HEADER_KERNELVERSION_LEN];
	uint64_t eph_platformECID;
	uint64_t eph_time_secs;
	uint64_t eph_time_usecs;
	...
	panic_info_tracker_t eph_panic_info_tracker;                  /* Panic timing and coprocessor status tracker */
} __attribute__((packed));

struct platform_information {
	uint32_t soc_id;
	uint32_t soc_rev;
	char product[EMBEDDED_PANIC_HEADER_PRODUCT_LEN];
	uint64_t platformECID;
	uint8_t status;
} __attribute__((packed));
