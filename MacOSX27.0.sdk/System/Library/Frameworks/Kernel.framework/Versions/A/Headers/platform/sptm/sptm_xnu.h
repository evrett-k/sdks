...
#define SPTM_FUNCTIONID_OUTPUT_AREA    50
...
__enum_closed_decl(sptm_iommu_id_enum_t,
    uint8_t,
    {
        IOMMU_ID_SHART = 0,
        IOMMU_ID_SART = 1,
        ...
        IOMMU_ID_DART_GEN3 = 8,

        SPTM_IOMMUS_N_IDS = 9,

        /* A placeholder ID to represent no/invalid IOMMU. */
        IOMMU_ID_INVALID = SPTM_IOMMUS_N_IDS,
    });
...
/**
 * TIndex constants for XNU execution modes.
 * These constants map Execution Modes to their associated TIndex values.
 */
enum {
	DECLARE_TINDEX(EXEC_MODE_XNU_DEFAULT),
	DECLARE_TINDEX(EXEC_MODE_XNU_ROZONE_RW),
	DECLARE_TINDEX(EXEC_MODE_XNU_USER_DEFAULT),
	DECLARE_TINDEX(EXEC_MODE_XNU_USER_JIT_RW),
	DECLARE_TINDEX(EXEC_MODE_XNU_USER_TPRO_RW),
};

/**
 * The number of TTEs in a subpage user root table.
 */
#define SUBPAGE_USER_ROOT_TABLE_TTES 128

/**
 * The size (in bytes) of a subpage user root table.
 *
 * Each SURT has 128 TTEs.
 */
#define SUBPAGE_USER_ROOT_TABLE_SIZE (SUBPAGE_USER_ROOT_TABLE_TTES * sizeof(sptm_pte_t))

#if XNU_CLIENT
/**
 * The number of subpage user root tables in a frame, which is also the number
 * of valid indexes into the array of subpage user root tables. The last index
 * is reserved for metadata like the FTE.
 */
#define SUBPAGE_USER_ROOT_TABLE_INDEXES ((PAGE_SIZE / SUBPAGE_USER_ROOT_TABLE_SIZE) - 1)
#endif /* XNU_CLIENT */
 ...
 *                 panic handler.
 * SPTM_CPU_EXCEPTION: Passed when SPTM has taken an exception and is handing
 *                 off to XNU for handling.
 */
 ...
 */
typedef struct {
/* Increment this every time you change the binary layout of this struct. */
#define SPTM_BOOTSTRAP_ARGS_XNU_VERSION 0xD00F000000000000ULL
	uint64_t version;
...
#if HAS_MTE
	/* MTE-related information */
	unsigned int n_tag_storage_frames;
	sptm_paddr_t first_tag_storage_paddr;
#endif /* HAS_MTE */
	...
	uint64_t timestamp_sptm_init;
	uint64_t timestamp_sk_complete;
	uint64_t timestamp_txm_complete;
 ...
 * Used by SEAR/LASER userspace tools and xnu tests, so keep in sync with them.
 */
typedef struct {
	sptm_paddr_t addr;
	size_t len;
...
#if DEVELOPMENT && HAS_MTE
/**
 * Function called by XNU to disable MTE via SCTLR.
 *
 * @note This is intended for XNU's KASAN-TBI to invoke before it starts tagging pointers,
 *       since MTE and KASAN-TBI are mutually exclusive. MTE may have been enabled before
 *       entering XNU, since we don't know if we're booting a KASAN variant or not.
 * @note This must be called prior to machine lockdown.
 * @note This is temporary: rdar://152648287 (Remove SPTM API to disable MTE after boot)
 */
void sptm_disable_mte(void);
#endif /* DEVELOPMENT && HAS_MTE */
 ...
 *
 * @note Only supported on XNU generic pages. The TLBI will be issued
...
/**
 * Update the in-memory PTE, but do not issue DSB+ISB after the TLBI.
 *
 * @note Only supported by sptm_update_disjoint(). It will be ignored by
 *       sptm_update_region() or the multipage APIs. The only known place
 *       this is useful is on the SPTM_UPDATE_AF path where it is okay to
 *       not sync the TLBI immediately after the disjoint update.
 *
 * @note Only supported on XNU generic pages. The DSB will be issued
 *       unconditionally for unsupported types of pages.
 */
#define SPTM_UPDATE_ASYNC_TLBI 0x400U
...
/**
 * Read a given virtual address [vaddr] to verify a fault.
 *
 * @param paddr Physical address of the page to read from.
 * @param vaddr Virtual address that maps to [paddr].
 * @param fault_expected True if a fault is expected during the read, False otherwise.
 *                       This is simply a hint to the SPTM that an exception should be expected.
 */
#if !USE_UNSAFE_TYPES
int sptm_test_read(sptm_paddr_t paddr, sptm_vaddr_t vaddr, bool fault_expected);
#else
int sptm_test_read(sptm_managed_page_u paddr_U, sptm_aligned_vaddr_u vaddr_U, bool fault_expected);
#endif
...
/**
 * Obtain the PAPT virtual address of the SPTM per-CPU output area for a
 * given SPTM logical CPU ID.
 *
 * XNU is expected to call this function during early boot to discover each
 * physical CPU's output area address, and is expected to cache the result
 * for later use.  XNU requires read-only access to the per-CPU output area
 * in order to process output from SPTM calls.  An example of this is the
 * 'prev_ptes' array generated by several SPTM calls.  Such output will
 * always be stored from the beginning of the output area.
 *
 * @param sptm_cpu_id SPTM logical CPU ID, obtained from a previous call to
 *                    sptm_cpu_id().
 *
 * @return The PAPT virtual address of the output area for [sptm_cpu_id].
 */
sptm_papt_t sptm_output_area(uint16_t sptm_cpu_id);
 ...
 *       point).
 *
 * @param caller_return_addr The return address of the original caller.
 * @param thread_stack_paddr The stack of the TXM thread to resume.
 */
__attribute__((noreturn)) void txm_resume(uint64_t caller_return_addr, uint64_t thread_stack_paddr);
 ...
 * @param caller_return_addr The return address of the original caller.
 */
__attribute__((noreturn)) void sptm_resume_from_exception(uint64_t caller_return_addr);
 ...
 * @param caller_return_addr The return address of the original caller.
 */
__attribute__((noreturn)) void sk_resume(uint64_t caller_return_addr);
...
/**
 * Libsptm library function for checking whether a physical address is writable
 * (WO/RW) under the UAT's SAPT.
 *
 * @note This function only works on SPTM-managed memory.
 *
 * @note The accuracy of this function requires that no other mappings are made
 *       concurrently to the physcial address. It should be used only when the
 *       caller knows the page is free of references and will not be concurrently
 *       mapped.
 *
 * @param paddr The physical address of the managed page against which to check
 *              if marked writable in the SAPT.
 * @param is_sapt_writable An output parameter used to store whether or not the page
 *                         is writable (WO/RW) in the SAPT.
 *
 * @return LIBSPTM_SUCCESS if the SAPT could be checked correctly for [paddr].
 *         LIBSPTM_INVALID_ARG if [paddr] is not in managed DRAM
 *                             or if [is_sapt_writable] is NULL.
 *         LIBSPTM_FAILURE if there is no SAPT table address.
 */
libsptm_error_t sptm_check_sapt_is_writable(sptm_paddr_t paddr, bool *is_sapt_writable);
...
/**
 * Returns whether the given paddr is associated with one of the SK shared
 * types and should be mapped with the RT cache attribute.
 *
 * @param paddr Input physical address
 * @param is_sk_shared_realtime Output pointer to a boolean indicating the
 *                              result of the query.
 * @return LIBSPTM_SUCCESS if the `is_sk_shared_realtime` pointer is
 *                         successfully set.
 *         LIBSPTM_INVALID_ARG if `is_sk_shared_realtime` is NULL.
 */
libsptm_error_t sptm_page_is_sk_shared_realtime(sptm_paddr_t paddr, bool *is_sk_shared_realtime);
#endif /* __ASSEMBLER__ */
