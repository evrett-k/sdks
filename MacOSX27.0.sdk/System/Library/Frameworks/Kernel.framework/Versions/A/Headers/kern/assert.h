...
#if MACH_ASSERT
/*
 * Unit test builds (__BUILDING_XNU_LIBRARY__) avoid static_if constructs because
 * they add the __BOOTDATA segment to executables, which conflicts with the
 * __DATA_POOL segment used for the mock_mem allocator in unit tests.
 * See tests/unit/Makefile for __DATA_POOL usage.
 */
