...
#define STACKSHOT_KCTYPE_TASK_SNAPSHOT               0x905u /* task_snapshot_v2, task_snapshot_v3, task_snapshot_v4 */
...
#define STACKSHOT_KCTYPE_MEMINFO_SNAPSHOT             0x95du /* struct meminfo_snapshot_v1 */
#define STACKSHOT_KCTYPE_SCHEDULER_STATE             0x95eu /* struct stackshot_scheduler_state */
...
// Those values need to mirror the VM_PAGER values in osfmk/vm/vm_pageout.h
enum mis_compressor_mode {
	MIS_VM_PAGER_NOT_CONFIGURED       = 0x00,
	MIS_VM_PAGER_COMPRESSOR_NO_SWAP   = 0x02,
	MIS_VM_PAGER_COMPRESSOR_WITH_SWAP = 0x04,
	MIS_VM_PAGER_FREEZER_NO_SWAP      = 0x10,
	MIS_VM_PAGER_FREEZER_WITH_SWAP    = 0x20,
};

struct meminfo_snapshot_v1 {
	uint32_t mis_booter_count;
	uint32_t mis_exclaves_carveout_count;
	uint32_t mis_exclaves_bundle_count;
	uint32_t mis_vm_page_kernelcache_count;
	uint32_t mis_vm_page_stolen_count;
	uint32_t mis_vm_page_inactive_target;
	uint32_t mis_vm_page_free_target;
	uint32_t mis_speculative_q_age_ms;
	uint32_t mis_cseg_limit;
	uint32_t mis_cseg_incore_limit;
	uint32_t mis_compressed_pages_limit;
	uint32_t mis_cseg_size;
	uint8_t  mis_speculative_percentage;
	uint8_t  mis_compressor_mode;
} __attribute__((packed));
...
struct stackshot_scheduler_state {
	uint64_t recommended_cores; /* cpumap of cores recommended for scheduling */
	uint64_t online_cores;      /* cpumap of cores currently online (powered) */
} __attribute__((packed));
...
#define TASK_CRASHINFO_PROC_PSA_FLAGS                           0x848 /* uint16_t */
