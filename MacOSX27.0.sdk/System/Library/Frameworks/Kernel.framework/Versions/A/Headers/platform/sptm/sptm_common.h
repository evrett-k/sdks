...
/* SPTM Dispatch Command IDs */
#define SPTM_DISPATCH_CMD_DISPATCH_CALL         0
#define SPTM_DISPATCH_CMD_RETURN_TO_CALLER      1
#define SPTM_DISPATCH_CMD_PANIC                 2
#define SPTM_DISPATCH_CMD_EXCEPTION_STATE_SAVED 3
#define SPTM_DISPATCH_CMD_RESUME_SPTM           4
#define SPTM_DISPATCH_CMD_RESUME_TXM            5
#define SPTM_DISPATCH_CMD_RESUME_SK             6
#define NUM_DISPATCH_CMDS                       7
...
#define SPTM_DISPATCH_TABLE_RESERVED1 9
...
#define SPTM_DISPATCH_TABLE_T8020_DART_XNU 14
#if defined(VBT_COMPONENT) || defined(SPTM_VBT)
#define SPTM_DISPATCH_TABLE_VBT 15
#else
#define SPTM_DISPATCH_TABLE_RESERVED2 15
#endif /* defined(VBT_COMPONENT) || defined(SPTM_VBT) */

#define SPTM_DISPATCH_TABLE_INVALID 16
...
#define SPTM_FEATURE_SAPT_REVOKE           (1ULL << 7)
...
/* Default page size for the current hardware. */
#define SPTM_PAGE_SHIFT 14U
#define SPTM_PAGE_SIZE  (1ULL << SPTM_PAGE_SHIFT)
#define SPTM_PAGE_MASK  (SPTM_PAGE_SIZE - 1ULL)
...
/* Helper macros to pack/extract command/domain/table/endpoint IDs */

#define DISPATCH_CMD_SHIFT 56
#define DISPATCH_CMD_WIDTH 8
#define EXTRACT_DISPATCH_CMD(x) \
	((sptm_dispatch_cmd_t)EXTRACT_BITFIELD(DISPATCH_CMD_WIDTH, DISPATCH_CMD_SHIFT, (x)))
#define PACK_DISPATCH_CMD(x) PACK_BITFIELD(DISPATCH_CMD_WIDTH, DISPATCH_CMD_SHIFT, (x))
...
#define BUILD_DISPATCH_CALL(domain, table, endpoint)                               \
	(PACK_DISPATCH_CMD(SPTM_DISPATCH_CMD_DISPATCH_CALL) | PACK_DOMAIN_ID(domain) | \
	    PACK_TABLE_ID(table) | PACK_ENDPOINT_ID(endpoint))

/* Helper macros for dispatch ID construction and stringification helpers for GEN_NAMED_STUB */
#define _STRINGIFY(A) #A
#define _XSTR(A)      _STRINGIFY(A)
#define GEN_DISPATCH_CALL_STR(domain, table, endpoint) \
	_XSTR(BUILD_DISPATCH_CALL(domain, table, endpoint))
#define GEN_DISPATCH_CMD_STR(command) _XSTR(PACK_DISPATCH_CMD(command))
...
/**
 * Assembler macro for loading the SPTM call register with the dispatch call ID
 *
 ...
 */
.macro SPTM_LOAD_DISPATCH_CALL domain_ID, table_ID, endpoint_ID
	mov		x16, #(BUILD_DISPATCH_CALL(\domain_ID, \table_ID, \endpoint_ID) & 0xffff)
	MOVK_IFNZ	x16, BUILD_DISPATCH_CALL(\domain_ID, \table_ID, \endpoint_ID), 16
	MOVK_IFNZ	x16, BUILD_DISPATCH_CALL(\domain_ID, \table_ID, \endpoint_ID), 32
	MOVK_IFNZ	x16, BUILD_DISPATCH_CALL(\domain_ID, \table_ID, \endpoint_ID), 48
.endm

/**
 * Assembler macro for loading the SPTM call register with a dispatch command other than CALL.
 *
 * @param command_ID The dispatch command ID
 */
.macro SPTM_LOAD_DISPATCH_CMD command_ID
		mov x16, #(PACK_DISPATCH_CMD(\command_ID))
.endm
#else /* __ASSEMBLER__ */
/* Helper macro for the stub-generating macros defined below */
#define GEN_LOAD_DISPATCH_CMD_CODE_SEQUENCE(cmd_ID) \
	"mov x16, #(" GEN_DISPATCH_CMD_STR(cmd_ID) ")\n"

#define GEN_LOAD_DISPATCH_CALL_CODE_SEQUENCE(domain_ID, table_ID, endpoint_ID) \
	 "movk x16, #(((" GEN_DISPATCH_CALL_STR(domain_ID, table_ID, endpoint_ID)  ") >> 48) & 0xFFFF), lsl #48\n" \
	 "movk x16, #(((" GEN_DISPATCH_CALL_STR(domain_ID, table_ID, endpoint_ID)  ") >> 32) & 0xFFFF), lsl #32\n" \
	 "movk x16, #(((" GEN_DISPATCH_CALL_STR(domain_ID, table_ID, endpoint_ID)  ") >> 16) & 0xFFFF), lsl #16\n" \
	 "movk x16, #(((" GEN_DISPATCH_CALL_STR(domain_ID, table_ID, endpoint_ID)  ") >> 00) & 0xFFFF)\n"
#endif /* __ASSEMBLER__ */
 ...
 *        sequence.
 * @param command_ID ID of the command (domain_ID, table_ID, endpoint_ID must be
 *                   none)
 *
 ...
 *
 * @note If command_ID is set, other *_ID fields must be `none`, and this
 *       generates an SPTM command, otherwise domain_ID and table_ID must
 *       be valid integers and this synthesizes a call.
 *
 ...
 *       to program x16 with.
 */
.macro GEN_DOMAIN_ENTRY_STUB stub_name, domain_name, has_hooks, domain_ID, table_ID, endpoint_ID=none, command_ID=none
	.align 2
	.global _\stub_name
_\stub_name:
	/*
...
.ifnc \command_ID,none

	/**
	 * SPTM Command
	 */
	.ifnc \domain_ID\table_ID\endpoint_ID,nonenonenone
	.error "When command_ID is set, domain_ID, table_ID and endpoint_ID must be none"
	.endif

	SPTM_LOAD_DISPATCH_CMD \command_ID

.else /* command_ID == none */
	.ifc \domain_ID,none
	.error "domain_ID and table_ID must be set (if command_ID is not)"
	.endif
	.ifc \table_ID,none
	.error "domain_ID and table_ID must be set (if command_ID is not)"
	.endif
...
.ifnc \endpoint_ID,none
	...
	SPTM_LOAD_DISPATCH_CALL \domain_ID, \table_ID, \endpoint_ID
...
.else /* endpoint_ID == none */
	...
	mov		w16, w0
	MOVK_IFNZ	x16, BUILD_DISPATCH_CALL(\domain_ID, \table_ID, 0), 32
	MOVK_IFNZ	x16, BUILD_DISPATCH_CALL(\domain_ID, \table_ID, 0), 48
...
.macro GEN_NAMED_SPTM_CMD_STUB function_name, cmd_ID
	.section __TEXT_BOOT_EXEC, __bootcode, regular, pure_instructions
	GEN_DOMAIN_ENTRY_STUB \function_name, "sptm", yes, none, none, none, \cmd_ID
.endm
 ...
 */
.macro GEN_NAMED_SPTM_DISPATCH_STUB function_name, table_ID, endpoint_ID
	.text
 ...
 *         is in accordance with the 64-bit AAPCS.
 */
.macro GEN_NAMED_SPTM_GUEST_DISPATCH_STUB function_name, table_ID, endpoint_ID
	.text
 ...
 *         is in accordance with the 64-bit AAPCS.
 */
.macro GEN_NAMED_SPTM_DISPATCH_STUB_BOOTEXEC function_name, table_ID, endpoint_ID
	.section __TEXT_BOOT_EXEC, __bootcode, regular, pure_instructions
	GEN_DOMAIN_ENTRY_STUB \function_name, "sptm", yes, SPTM_DOMAIN, \table_ID, \endpoint_ID
.endm
...
typedef uint8_t sptm_dispatch_cmd_t;
...
/**
 * Convert an execution_mode_t to its associated TIndex value.
 *
 * This allows the hardware to have separate TIndex spaces for privileged
 * and user modes while the SPTM uses a simple sequential enum to represent
 * them.
 */
#define EXEC_MODE_TINDEX(mode) \
	((mode) < FIRST_USER_EXECUTION_MODE ? (mode) : ((mode) - FIRST_USER_EXECUTION_MODE))

/**
 * Helper macro to define TINDEX constants for execution modes.
 * Automatically generates a constant named <mode>_TINDEX with the computed TINDEX value.
 */
#define DECLARE_TINDEX(mode) mode##_TINDEX = EXEC_MODE_TINDEX(mode)
 ...
 *
 * @note If updating this enum, ensure the #defines mimicking this enum higher
 *       up in this file (for assembly code) are updated to match.
 */
...
#define LIBSPTM_VERSION_11             11U
#define LIBSPTM_VERSION_12             12U
#define LIBSPTM_VERSION                LIBSPTM_VERSION_12
...
#define LIBSPTM_CPU_FEATURE_USERAT     1U
	...
	/* Version 11 fields: */
	sptm_paddr_t dram_base_paddr;
	sptm_paddr_t dram_end_paddr;
	sptm_papt_t sapt_table_papt;
	/* End of Version 11 fields */

	/* Version 12 fields: */
	const char *version_string;

#if defined(SPTM_VBT) || defined(VBT_COMPONENT)
	/**
	 * Only when building for the Virtualization Based Testing SPTM binary do we
	 * expose out the SPTM's per-cpu data. On all real systems, don't directly
	 * expose this.
	 *
	 * The number of per-cpu data entries provided by this array is determined
	 * by `max_cpus` above.
	 */
	const void *sptm_percpu_data;
#else
	uint8_t reserved0[8];
#endif /* defined(SPTM_VBT) || defined(VBT_COMPONENT)*/
	/* End of Version 12 fields */
...
#define SPTM_SK_PAGE_FLAGS_TAGGABLE 0x1
		...
		/**
		 * May be set at retype time by SK in the 'sk_flags' field of the retype_params
		 * to indicate that this frame is expected to be mapped as RT/TrueUC.
		 */
#define SPTM_SK_PAGE_FLAGS_RT       0x2

#define SPTM_SK_PAGE_FLAGS_ALL (SPTM_SK_PAGE_FLAGS_TAGGABLE | SPTM_SK_PAGE_FLAGS_RT)
...
/**
 * Structure produced by SPTM and given to XNU in order to properly handle a
 * guest exception. SPTM "hands off" this structure so that XNU can fix up its
 * own register state to make it appear as if the guest took an exception
 * directly to XNU.
 */
typedef struct {
	uint64_t x0;
	...
	uint64_t spsr_el2;
	uint64_t aspsr_el2;
} sptm_guest_exception_handoff_t;
 ...
 */
/* clang-format off */
#define GEN_NAMED_DISPATCH_STUB(location_directive, function_name, entry_stub_name, domain_ID, table_ID, endpoint_ID) \
	__asm__ ( location_directive " \n" \
			 ".align 2 \n" \
			 ".globl _" #function_name "\n" \
			 "_" #function_name ":\n" \
			 "bti c\n" \
			 GEN_LOAD_DISPATCH_CALL_CODE_SEQUENCE(domain_ID, table_ID, endpoint_ID) \
			 "b _" entry_stub_name "\n")

#if defined(XNU_CLIENT) || defined(TXM_CLIENT) || defined(SK_CLIENT) || defined(SPTM_INTERNAL)
#define GEN_NAMED_CMD_STUB(location_directive, function_name, entry_stub_name, cmd_ID) \
	__asm__ ( location_directive " \n" \
			 ".align 2 \n" \
			 ".globl _" #function_name "\n" \
			 "_" #function_name ":\n" \
			 "bti c\n" \
			 GEN_LOAD_DISPATCH_CMD_CODE_SEQUENCE(cmd_ID) \
			 "b _" entry_stub_name "\n")
/* clang-format on */
#endif /* defined(XNU_CLIENT) || defined(TXM_CLIENT) || defined(SK_CLIENT) || defined(SPTM_INTERNAL) */
...
/**
 * Retrieves the SPTM version string.
 *
 * @param version_string Pointer to a const char pointer that will be set to the
 *        SPTM version string.
 *
 * @return LIBSPTM_SUCCESS if the version string is available
 *         LIBSPTM_NOT_INITTED if libsptm is not yet initialized
 *         LIBSPTM_INVALID_ARG if version_string is NULL
 *         LIBSPTM_FAILURE if the version string is not available
 */
extern libsptm_error_t __attribute__((__warn_unused_result__)) sptm_get_version_string(
    const char **version_string);
