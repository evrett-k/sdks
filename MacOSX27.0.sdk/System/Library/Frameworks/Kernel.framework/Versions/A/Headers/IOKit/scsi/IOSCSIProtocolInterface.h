...
#if (defined(KERNEL) || defined(IOSAM_UNIT_TEST)) && defined(__cplusplus)

#ifdef IOSAM_UNIT_TEST
// In the kernel, `kCFBundleIdentifierKey` is available, but when building for
// userspace testing, it conflicts with a definition in CFBundle.h
/*!
@constant kCFSCSIBundleIdentifierKey
@discussion
Property key for CFBundleIdentifier.
*/
#define kCFSCSIBundleIdentifierKey                      "CFBundleIdentifier"
#else
/*!
...
protected:
	...
	*/
#ifdef IOSAM_UNIT_TEST
	// Hack for now, we should add finalize to IOKitty's IOService implementation
...
#endif	/* (defined(KERNEL) || defined(IOSAM_UNIT_TEST)) && defined(__cplusplus) */
