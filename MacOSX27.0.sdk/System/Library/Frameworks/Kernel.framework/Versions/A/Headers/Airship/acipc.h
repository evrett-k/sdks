...
typedef enum airship_acipc_ring_state_detail_flag
{
    ...
    /// The ring dependencies are not available.
    ...
    /// The ring was closed because the auxiliary context used has closed.
    AIRSHIP_ACIPC_RING_STATE_DEAD_CONTEXT = (1 << 7),
...
/**
 * @brief Returns the doorbell associated with a transfer ring.
 * @param tr The transfer ring to be accessed.
 * @param out_selector Output param used to return the numerical selector of the
 * associated doorbell, or zero if `NULL` is returned.
 * @returns The name of the associated doorbell, or `NULL` on error or if the
 * transfer ring is not associated with any doorbell. The caller is responsible
 * for releasing the returned string.
 */
AIRSHIP_EXPORT
AIRSHIP_RETURNS_RETAINED airship_string_t AIRSHIP_NULLABLE airship_acipc_tr_get_doorbell(airship_acipc_tr_t tr,
                                                                                         AIRSHIP_NOESCAPE uint32_t *AIRSHIP_NULLABLE out_selector);
...
/**
 * @brief Returns the interrupt associated with a transfer ring.
 * @param tr The transfer ring to be accessed.
 * @param out_selector Output param used to return the numerical selector of the
 * associated interrupt, or zero if `NULL` is returned.
 * @returns The name of the associated interrupt, or `NULL` on error or if the
 * transfer ring is not associated with any interrupt. The caller is responsible
 * for releasing the returned string.
 */
AIRSHIP_EXPORT
AIRSHIP_RETURNS_RETAINED airship_string_t AIRSHIP_NULLABLE airship_acipc_tr_get_interrupt(airship_acipc_tr_t tr,
                                                                                          AIRSHIP_NOESCAPE uint32_t *AIRSHIP_NULLABLE out_selector);
...
/**
 * @brief Computes required backing memory size for a transfer ring.
 * @param tr The transfer ring to be accessed.
 * @param out_alignment Output parameter used to return the minimum required
 * alignment for the backing memory.
 * @returns The size of the backing memory required for the ring.
 */
AIRSHIP_EXPORT
uint32_t airship_acipc_tr_compute_required_backing_memory_size(airship_acipc_tr_t tr, uint32_t *AIRSHIP_NULLABLE out_alignment);

/**
 * @brief Configures the backing memory for a transfer ring.
 *
 * This function may not be invoked after activation.
 *
 * @param tr The transfer ring to be accessed.
 * @param buffer The backing memory buffer to use for the ring.
 * @param offset The offset in bytes at which the ring should begin in the
 * backing memory buffer.
 * @param length The maximum length in bytes of the ring in the backing memory
 * buffer.
 */
AIRSHIP_EXPORT
void airship_acipc_tr_set_backing_memory(airship_acipc_tr_t tr, airship_buffer_t AIRSHIP_NULLABLE buffer, uint32_t offset, uint32_t length);

/**
 * @brief Configures a transfer ring for external management.
 *
 * This function may not be invoked after activation.
 *
 * Externally-managed rings participate in ring lifecycle messaging but do not
 * support enqueueing transfers. Associated index array elements are initialized
 * to zero when the ring is opened but are never otherwise accessed.
 *
 * @param tr The transfer ring to be accessed.
 * @param external_management Whether the ring is externally managed.
 */
AIRSHIP_EXPORT
void airship_acipc_tr_set_external_management(airship_acipc_tr_t tr, bool external_management);

/**
 * @brief Returns the index array vector associated with a transfer ring.
 * @param tr The transfer ring to be accessed.
 * @returns The index array vector associated with the transfer ring.
 */
AIRSHIP_EXPORT
uint16_t airship_acipc_tr_get_index_array_vector(airship_acipc_tr_t tr);
 ...
 * - `::airship_acipc_tr_set_backing_memory`
 * - `::airship_acipc_tr_set_external_management`
 *
...
/**
 * @brief Returns the doorbell associated with a completion ring.
 * @param cr The completion ring to be accessed.
 * @param out_selector Output param used to return the numerical selector of the
 * associated doorbell, or zero if `NULL` is returned.
 * @returns The name of the associated doorbell, or `NULL` on error or if the
 * completion ring is not associated with any doorbell. The caller is
 * responsible for releasing the returned string.
 */
AIRSHIP_EXPORT
AIRSHIP_RETURNS_RETAINED airship_string_t AIRSHIP_NULLABLE airship_acipc_cr_get_doorbell(airship_acipc_cr_t cr,
                                                                                         AIRSHIP_NOESCAPE uint32_t *AIRSHIP_NULLABLE out_selector);
...
/**
 * @brief Returns the interrupt associated with a completion ring.
 * @param cr The completion ring to be accessed.
 * @param out_selector Output param used to return the numerical selector of the
 * associated interrupt, or zero if `NULL` is returned.
 * @returns The name of the associated interrupt, or `NULL` on error or if the
 * completion ring is not associated with any interrupt. The caller is
 * responsible for releasing the returned string.
 */
AIRSHIP_EXPORT
AIRSHIP_RETURNS_RETAINED airship_string_t AIRSHIP_NULLABLE airship_acipc_cr_get_interrupt(airship_acipc_cr_t cr,
                                                                                          AIRSHIP_NOESCAPE uint32_t *AIRSHIP_NULLABLE out_selector);
...
/**
 * @brief Computes required backing memory size for a completion ring.
 * @param cr The completion ring to be accessed.
 * @param out_alignment Output parameter used to return the minimum required
 * alignment for the backing memory.
 * @returns The size of the backing memory required for the ring.
 */
AIRSHIP_EXPORT
uint32_t airship_acipc_cr_compute_required_backing_memory_size(airship_acipc_cr_t cr, uint32_t *AIRSHIP_NULLABLE out_alignment);

/**
 * @brief Configures the backing memory for a completion ring.
 *
 * This function may not be invoked after activation.
 *
 * @param cr The completion ring to be accessed.
 ...
 */
AIRSHIP_EXPORT
void airship_acipc_cr_set_backing_memory(airship_acipc_cr_t cr, airship_buffer_t AIRSHIP_NULLABLE buffer, uint32_t offset, uint32_t length);

/**
 * @brief Configures a completion ring for external management.
 *
 * This function may not be invoked after activation.
 *
 * Externally-managed rings participate in ring lifecycle messaging but do not
 * support receiving completions. Associated index array elements are
 * initialized to zero when the ring is opened but are never otherwise accessed.
 *
 * @param cr The completion ring to be accessed.
 * @param external_management Whether the ring is externally managed.
 */
AIRSHIP_EXPORT
void airship_acipc_cr_set_external_management(airship_acipc_cr_t cr, bool external_management);

/**
 * @brief Returns the index array vector associated with a completion ring.
 * @param cr The completion ring to be accessed.
 * @returns The index array vector associated with the completion ring.
 */
AIRSHIP_EXPORT
uint16_t airship_acipc_cr_get_index_array_vector(airship_acipc_cr_t cr);
 ...
 * - `::airship_acipc_cr_set_doorbell_moderation`
 * - `::airship_acipc_cr_set_interrupt`
 * - `::airship_acipc_cr_set_backing_memory`
 * - `::airship_acipc_cr_set_external_management`
 *
...
/**
 * @brief Auxiliary context states returned by
 * `::airship_acipc_auxiliary_context_get_state`.
 *
 * ```
 *  -·-·- Autonomous transition
 *  ----- Directed transition
 ...
 * ```
 */
typedef enum airship_acipc_auxiliary_context_state
{
    /**
     * @brief The auxiliary context is closed and not available to be opened.
     *
     * See `::airship_acipc_auxiliary_context_state_detail_flag_t` for reasons a context may be
     * unavailable.
     *
     * Allowed transitions:
     * - `AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_AVAILABLE` if the context becomes available.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE,
    /**
     * @brief The auxiliary context is closed and available to be opened.
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_OPENING` with `::airship_acipc_auxiliary_context_open`.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE` if the context becomes unavailable.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_AVAILABLE,
    /**
     * @brief The auxiliary context is awaiting the device's response to an open message.
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_OPEN` if the device acknowledges the open
     * message.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_UNSETTLED` if the context becomes unavailable
     * of the device rejects the open message.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_CLOSING` with `::airship_acipc_auxiliary_context_close`.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_OPENING,
    /**
     * @brief The auxiliary context is open and ready for use.
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_CLOSING` with `::airship_acipc_auxiliary_context_close`.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_UNSETTLED` if the context becomes unavailable.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_OPEN,
    /**
     * @brief The auxiliary context is awaiting the device's response to a close message.
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_UNSETTLED` if the context becomes unavailable
     * or if the device acknowledges the close message.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_LIMBO` if the device rejects the close message.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_CLOSING,
    /**
     * @brief The device has rejected a close message for the context. The context is not
     * usable.
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_CLOSING` with `::airship_acipc_auxiliary_context_close`.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_UNSETTLED` if the ring becomes unavailable.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_LIMBO,
    /**
     * @brief The auxiliary context is closed, but it is not yet safe to reset its state.
     *
     * See `::airship_acipc_auxiliary_context_state_detail_flag_t` for reasons a context may become
     * "dead".
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_CONTET_STATE_DEAD_SETTLED` when it becomes safe to reset the
     * context state.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_UNSETTLED,
    /**
     * @brief The auxiliary context is closed, and its state can be safely reset.
     *
     * See `::airship_acipc_auxiliary_context_state_detail_flag_t` for reasons a context may become
     * "dead".
     *
     * Allowed transitions:
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_AVAILABLE` with `::airship_acipc_auxiliary_context_reset`.
     * - `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE` with `::airship_acipc_auxiliary_context_reset`.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_SETTLED,
    /**
     * @brief Value returned by `::airship_acipc_auxiliary_context_get_state` if the connection to
     * the driver is broken.
     */
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_EDISCONNECTED,
} airship_acipc_auxiliary_context_state_t;

/**
 * @brief Detailed status flags returned by `::airship_acipc_auxiliary_context_get_state`.
 */
typedef enum airship_acipc_auxiliary_context_state_detail_flag
{
    /// The device is not in the appropriate execution stage.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE_EXECSTAGE = (1 << 0),
    /// The auxiliary context parameters conflict with a context that is already open.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE_CONFLICT = (1 << 1),
    /// The auxiliary context dependencies are not available.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE_DEPENDENCY = (1 << 2),
    /// The device rejected the open message.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_OPENREJECT = (1 << 3),
    /// The auxiliary context was explicitly closed by `::airship_acipc_auxiliary_context_close`.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_BYREQUEST = (1 << 4),
    /// The auxiliary context was closed because the driver entered reset.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_DEVICERESET = (1 << 5),
    /// The auxiliary context was closed because the device execution stage changed.
    AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_EXECSTAGE = (1 << 6),
} airship_acipc_auxiliary_context_state_detail_flag_t;

/// ACIPC context handle type.
typedef struct airship_acipc_auxiliary_context *airship_acipc_auxiliary_context_t;

/**
 * @brief Create a handle for an auxiliary context.
 *
 * The returned handle must be released with
 * `::airship_acipc_auxiliary_context_destroy`.
 *
 * @param client A client handle connected to the driver.
 * @param name The name of the auxiliary context to be created.
 * @param selector The selector of the auxiliary context to be created.
 * @returns The newly created auxiliary context handle, or `NULL` if an error occurs.
 */
AIRSHIP_EXPORT
airship_acipc_auxiliary_context_t AIRSHIP_NULLABLE airship_acipc_auxiliary_context_create(airship_client_t client, airship_string_t name, uint32_t selector);

/**
 * @brief Destroys a auxiliary context handle, releasing any associated resources.
 * @param context The auxiliary context handle to be destroyed.
 */
AIRSHIP_EXPORT
void airship_acipc_auxiliary_context_destroy(airship_acipc_auxiliary_context_t context);

/**
 * @brief Returns the maximum TR index array vector for an auxiliary context.
 * @param context The auxiliary context to be accessed.
 * @returns The maximum transfer ring index array vector allowed by the context.
 */
AIRSHIP_EXPORT
uint16_t airship_acipc_auxiliary_context_get_tr_index_count(airship_acipc_auxiliary_context_t context);

/**
 * @brief Returns the maximum CR index array vector for an auxiliary context.
 * @param context The auxiliary context to be accessed.
 * @returns The maximum completion ring index array vector allowed by the
 * context.
 */
AIRSHIP_EXPORT
uint16_t airship_acipc_auxiliary_context_get_cr_index_count(airship_acipc_auxiliary_context_t context);

/**
 * @brief Returns the peripheral info interrupt for an auxiliary context.
 * @param context The auxiliary context to be accessed.
 * @param out_selector Output param used to return the numerical selector of the
 * associated interrupt, or zero if `NULL` is returned.
 * @returns The name of the peripheral info interrupt, or `NULL` on error or if
 * the does not have any peripheral info interrupt. The caller is responsible
 * for releasing the returned string.
 */
AIRSHIP_EXPORT
AIRSHIP_RETURNS_RETAINED airship_string_t AIRSHIP_NULLABLE airship_acipc_auxiliary_context_get_peripheral_info_interrupt(
    airship_acipc_auxiliary_context_t context, AIRSHIP_NOESCAPE uint32_t *AIRSHIP_NULLABLE out_selector);

/**
 * @brief Set the backing memory buffer for the auxiliary context to use.
 *
 * This function is not required to set up an auxiliary context,
 * if it is not used, ACIPC will manage the auxiliary context buffer.
 *
 * @param context The auxiliary context to access.
 * @param buffer The airship buffer object to use.
 * @param offset The offset within `buffer` to use as the start address.
 * @param length The maximum length of the context info within `buffer`.
 */
AIRSHIP_EXPORT
void airship_acipc_auxiliary_context_set_backing_memory(airship_acipc_auxiliary_context_t context, airship_buffer_t AIRSHIP_NULLABLE buffer, uint32_t offset,
                                                        uint32_t length);

/**
 * @brief Activate an auxiliary context.
 * @param context The auxiliary context to activate.
 * @retval ::AIRSHIP_OK if the region was successfully activated.
 * @retval ::AIRSHIP_EDISCONNECTED if the connection to the driver is broken.
 * @retval ::AIRSHIP_EBADARGUMENT if the context configuration is invalid.
 */
AIRSHIP_EXPORT
airship_result_t airship_acipc_auxiliary_context_activate(airship_acipc_auxiliary_context_t context);

/**
 * @brief Retrieves an auxiliary context's current state.
 * @param context The auxiliary context to be accessed.
 * @param out_detail Optional output parameter used to return detailed status
 * information. The returned value will be a bitmask of values from
 * `airship_acipc_auxiliary_context_state_detail_flag_t`.
 * @returns The auxiliary context's current state.
 */
AIRSHIP_EXPORT
airship_acipc_auxiliary_context_state_t airship_acipc_auxiliary_context_get_state(airship_acipc_auxiliary_context_t context,
                                                                                  AIRSHIP_NOESCAPE uint32_t *AIRSHIP_NULLABLE out_detail);

/**
 * @brief Creates a monitor associated with an auxiliary context's state.
 *
 * The returned handle must be released with `::airship_monitor_destroy`.
 *
 * @param context The auxiliary context to be tracked.
 * @returns The newly created monitor handle, or `NULL` if an error occurs.
 */
AIRSHIP_EXPORT
airship_monitor_t AIRSHIP_NULLABLE airship_acipc_auxiliary_context_create_state_monitor(airship_acipc_auxiliary_context_t context);

/**
 * @brief Open an auxiliary context.
 *
 * This function only sends an *AuxiliaryContext Open* message; it does not wait for the
 * device's response.
 *
 * @param context The auxiliary context to open.
 * @retval ::AIRSHIP_OK if successful.
 * @retval ::AIRSHIP_EDISCONNECTED if the connection to the driver is broken.
 * @retval ::AIRSHIP_ENOTREADY if the auxiliary context state is not
 * `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_AVAILABLE`.
 */
AIRSHIP_EXPORT
airship_result_t airship_acipc_auxiliary_context_open(airship_acipc_auxiliary_context_t context);

/**
 * @brief Close an auxiliary context.
 *
 * This function only sends an *AuxiliaryContext Close* message; it does not wait for the
 * device's response.
 *
 * @param context The auxiliary context to close.
 * @retval ::AIRSHIP_OK if successful.
 * @retval ::AIRSHIP_EDISCONNECTED if the connection to the driver is broken.
 * @retval ::AIRSHIP_ENOTREADY if the auxiliary context state is not one of:
 * - `::AIRSHIP_ACIPC_RING_STATE_OPENING`
 * - `::AIRSHIP_ACIPC_RING_STATE_OPEN`
 * - `::AIRSHIP_ACIPC_RING_STATE_LIMBO`
 */
AIRSHIP_EXPORT
airship_result_t airship_acipc_auxiliary_context_close(airship_acipc_auxiliary_context_t context);

/**
 * @brief Resets an inactive auxiliary context.
 *
 * On reset:
 * - The auxiliary context state is changed to `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_AVAILABLE` or
 * `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_UNAVAILABLE`.
 *
 * @param context The boot context to be reset.
 * @retval ::AIRSHIP_OK if successful.
 * @retval ::AIRSHIP_EDISCONNECTED if the connection to the driver is broken.
 * @retval ::AIRSHIP_ENOTREADY if the boot auxiliary state is not
 * `::AIRSHIP_ACIPC_AUXILIARY_CONTEXT_STATE_DEAD_SETTLED`.
 */
AIRSHIP_EXPORT
airship_result_t airship_acipc_auxiliary_context_reset(airship_acipc_auxiliary_context_t context);

/**
 * @brief Struct populated by `airship_acipc_auxiliary_context_reset`, which contains the offset
 * information for a context handle.
 */
typedef struct airship_acipc_auxiliary_context_offsets
{
    /// The offset of the context info struct within the context's backing memory.
    uint32_t context_info;
    /// The offset of the peripheral info structure within the context's backing memory.
    uint32_t peripheral_info;
    /// The offset of the completion ring head index array within the context's backing memory.
    uint32_t cr_hia;
    /// The offset of the transfer ring tail index array within the context's backing memory.
    uint32_t tr_tia;
    /// The offset of the completion ring tail index array within the context's backing memory.
    uint32_t cr_tia;
    /// The offset of the transfer ring head index array within the context's backing memory.
    uint32_t tr_hia;
    /// The offset of the end on the context memory region, equal to the total length of the auxiliary context.
    uint32_t end;
} airship_acipc_auxiliary_context_offsets_t;

/**
 * @brief Populates a struct pointed to by `out_offsets` with byte-offets for each
 * component within an auxilairy context.
 *
 * @param context The auxiliary context handle to query.
 * @param out_offsets A pointer to an `airship_acipc_auxiliary_context_offsets_t` instance, which
 * will be populated.
 * @retval ::AIRSHIP_OK if successful.
 * @retval ::AIRSHIP_EBADARGUMENT if `out_offsets` is null.
 */
AIRSHIP_EXPORT
airship_result_t airship_acipc_auxiliary_context_get_offsets(airship_acipc_auxiliary_context_t context, airship_acipc_auxiliary_context_offsets_t *out_offsets);
...
/// C++ wrapper for automatic lifecycle management of doorbell handles.
using doorbell = wrapper<airship_acipc_doorbell, airship_acipc_doorbell_destroy>;
/// C++ wrapper for automatic lifecycle management of auxiliary context handles.
using auxiliary_context = wrapper<airship_acipc_auxiliary_context, airship_acipc_auxiliary_context_destroy>;
} // namespace airship::acipc
#endif
