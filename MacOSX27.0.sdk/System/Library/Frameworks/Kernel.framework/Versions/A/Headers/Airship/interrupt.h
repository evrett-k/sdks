...
/**
 * @brief Sets the handler to be invoked when an interrupt event occurs.
 *
 * Handler invocations are serialized, even if the execution context configured
 * with `::airship_interrupt_set_queue` is concurrent. If an interrupt event
 * occurs while the event handler is executing, the handler will be invoked
 * again after the current invocation returns.
 *
 * Multiple interrupt events may be coalesced into a single handler invocation.
 *
 * This function may not be called after the interrupt is activated.
 *
 ...
 *
 * @param interrupt The interrupt to be configured.
 * @param handler The handler to be invoked when an interrupt event occurs.
 */
AIRSHIP_EXPORT
