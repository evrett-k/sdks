 ...
 *              thrown.
 * @param       capacity Size of the buffer to allocate
 * @param       options IOUSBHostObjectDataOptions. Default value is IOUSBHostObjectDataOptionsNone.
 * @return      NSMutableData of memory mapped to user space of an IOBufferMemoryDescriptor if successful,
 *              otherwise nil. An IOReturn error code will be reported on failure. The result is
 *              to be released by the caller
 */
- (nullable NSMutableData*)dataWithCapacity:(NSUInteger)capacity
                                    options:(IOUSBHostObjectDataOptions)options
                                      error:(NSError* _Nullable*)error API_AVAILABLE(macos(26.4));
