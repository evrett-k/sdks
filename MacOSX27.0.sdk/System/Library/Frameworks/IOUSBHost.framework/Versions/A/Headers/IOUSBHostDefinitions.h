...
/*!
 * @enum        IOUSBHostObjectDataOptions
 * @brief       Options for <code>dataWithCapacity:options:error</code>
 * @constant    IOUSBHostObjectDataOptionsKernelUserShared The created NSMutableData will be mapped into the current
 *              task as well as the kernel.  This option is recommended when allocating isochronous frame lists.
 */
typedef NS_OPTIONS (NSUInteger, IOUSBHostObjectDataOptions)
{
    IOUSBHostObjectDataOptionsNone             = 0,
    IOUSBHostObjectDataOptionsKernelUserShared = (1 << 0)
};
