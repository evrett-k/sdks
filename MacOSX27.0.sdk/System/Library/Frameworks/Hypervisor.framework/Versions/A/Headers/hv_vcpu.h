...
/*!
 @abstract Gets pending SError for a vcpu.
 @param vcpu ID of the vcpu instance.
 @param pending Returns whether the SError is pending or not.
 @discussion
    Must be called by the owning thread.
 */
OS_EXPORT API_AVAILABLE(macos(27.0))
hv_return_t hv_vcpu_get_serror(hv_vcpu_t vcpu, bool *pending);

/*!
 @abstract Sets pending SError for a vcpu.
 @param vcpu ID of the vcpu instance.
 @param pending Whether the SError is pending or not.
 @discussion
    Must be called by the owning thread.
 */
OS_EXPORT API_AVAILABLE(macos(27.0))
...
/*!
 ...
 @discussion
    ...
    Returns HV_UNSUPPORTED if the VM was created with a GIC device (hv_gic_create).
 */
OS_EXPORT API_AVAILABLE(macos(11.0))
...
/*!
 @abstract Returns the cumulative wait time of a vCPU spent at WFI instruction while waiting for interrupts in the units of mach_absolute_time().
 @param vcpu ID of the vcpu instance.
 @param time Pointer to wait time value (written on success).
 @result HV_SUCCESS on success, error code otherwise.
 @discussion
    Must be called by the owning thread.
    Returns HV_UNSUPPORTED if the VM was created without a GIC device (hv_gic_create).
 */
OS_EXPORT API_AVAILABLE(macos(27.0))
...
/*!
 @abstract Invalidates TLB entries for the specified vCPU.
 @param vcpu ID of the vCPU instance.
 @param op TLB invalidation operation to perform.
 @param param Parameter for the TLB operation (e.g., virtual address for VAAE1IS).
 @result HV_SUCCESS on success, an error code otherwise.
 @discussion
    Must be called by the owning thread.
    When EL2 is enabled for this VM, this function invalidates TLB entries for the guest hypervisor, not the nested guests.
 */
OS_EXPORT API_AVAILABLE(macos(27.0))
